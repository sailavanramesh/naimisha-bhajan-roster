...
/**
 * Put somebody on a job for this session, or take them off it.
 *
 * Works on bhajan sessions as much as on programs — the person on the desk on
 * a Thursday is doing the same job with fewer channels — so this deliberately
 * does not check the session's format.
 */
export async function setSessionCrew(input: {
  sessionId: string;
  singerId: string;
  job: "soundEngineer" | "micCoordinator";
  on: boolean;
}): Promise<Result> {
  await requireCapability("editPrograms");
  ...
  const parsed = z
    .object({
      sessionId: Id,
      singerId: Id,
      job: z.enum(["soundEngineer", "micCoordinator"]),
      on: z.boolean(),
    })
    .safeParse(input);
  ...
  if (parsed.data.on) {
    await prisma.sessionCrew.upsert({
      where: key,
      create: {
        sessionId: parsed.data.sessionId,
        singerId: parsed.data.singerId,
    ...
    await prisma.sessionCrew.deleteMany({
      where: {
        sessionId: parsed.data.sessionId,
        singerId: parsed.data.singerId,
        job: parsed.data.job,
